...
```{r fig1}
# read in tree and fix tip labels
rpoB_tree <- ape::read.tree(file = here('data', 'rpoB.tree'))
rpoB_tree$tip.label <- gsub("\\.", ". ", rpoB_tree$tip.label)


pa <-
  read.csv(here('data', 'fig1_genes.csv')) |>
  column_to_rownames(var = 'X') |> # set path
  mutate(across(everything(), ~ as.character(ceiling(.x)))) |>
...
pa <-
  rbind(
  pa,
  rep(NA, times = ncol(pa)))

rownames(pa)[nrow(pa)] <- 'T. eurythermalis'

colnames(pa) <- gsub("\\.", " ", colnames(pa))

resp_pa <-
  pa |>
  dplyr::select(contains(c('COG1271', 'COG1294', 'COG4987', 'COG4988',
                           'COG1169', 'COG1165', 'COG0596', 'COG0447',
                           'COG0318', 'COG1441', 'COG1575', 'COG2226', 'COG1252')))  
oxstress_pa <-
  pa |>
  dplyr::select(!contains(c('COG1271', 'COG1294', 'COG4987', 'COG4988',
                            'COG1169', 'COG1165', 'COG0596', 'COG0447',
...
```{r}
small_mutation_table <- read.csv(here('data', 'small_mutation_table_WC.csv'))
...
# import long et al 2018 Lc. lactis spectrum

segwidth <- 3/6
longlactis <-
  read_excel(here('data', 'long_lactis_spectrum.xlsx')) |>
  mutate(species = 'l') |>
  pivot_longer(!starts_with('species'),
               values_to = 'mean') |>
  mutate(type = 
           case_when(
             str_detect(name, ">") ~ str_replace(name, '>', ' > '),
             name == 'SNM Rate' ~ 'SNM'
           ),
        Species = "*Lc. lactis* (Long 2018)",
...
fig_2a_data |>
  ...
  mutate(Species = factor(Species, levels = c("*Lb. acidophilus*", "*Lb. crispatus*", "*Lc. lactis*"), ordered = T)) |>

   ggplot(aes(x = type, group = Species)) +
      scale_y_log10(guide = "axis_logticks", labels = label_log(), limits = c(1e-11, 5e-8), expand = c(0, 0)) +
      scale_x_discrete() +
      scale_color_manual(values = c(ct[[1]], ct[[2]], ct[[3]], 'black')) +
  geom_segment(
    data = longlactis,
    aes(x = X,
        xend = XEND,
    ...
    guides(color = guide_legend(override.aes = list(linewidth = 5))) +
      ...
      geom_errorbar(aes(ymin = mean - (sem * 1.96),
                        ymax = mean + (sem * 1.96)),
                    color = 'black', width = 0.3,
                    linewidth = 0.6, position = position_dodge(width = 0.9)) +
...
import egglib diversity stats
```{r}
a_egg <- read_tsv(here('data', 'acidophilus_egglib_stats.tsv')) |> mutate(species = 'a')
c_egg <- read_tsv(here('data', 'crispatus_egglib_stats.tsv')) |> mutate(species = 'c')
l_egg <- read_tsv(here('data', 'lactis_egglib_stats.tsv')) |> mutate(species = 'l')

egg <- rbind(a_egg, c_egg) |> rbind(l_egg) |> filter(wholeGene_lseff > 0) # remove genes with no analyzable sites
...
# import pi data
#pi <- read.csv(here('data', 'pi_results.csv')) |> filter(source == 'all_samples')
...
fig_2b_data <-
  ...
  rowwise() |>
  mutate(
    Ne = (egg |> filter(species == sp, gene == 'CONCAT') |> pull(ffs_Pi_perSite)) / (2 * snm_per_site_per_gen) ) |>
      # case_when(
      #   sp == 'a' ~ (filter(egg, species == 'acidophilus') |> pull(n_and_length_normalized_sum_site_pi)) / (2 * snm_per_site_per_gen) ,
      #   sp == 'c' ~ (filter(pi, species == 'crispatus') |> pull(n_and_length_normalized_sum_site_pi)) / (2 * snm_per_site_per_gen) ,
      #   sp == 'l' ~ (filter(pi, species == 'lactis') |> pull(n_and_length_normalized_sum_site_pi)) / (2 * snm_per_site_per_gen) ,
      #   TRUE ~ NA)) |>
  ungroup() |>
  mutate(organism_type = 'Bacteria') |> 
...
fig_4b <-
  sv_rates |> 
  rowwise() |>
  mutate(ci_95 = per_genome_per_gen_se * 1.96,
         ci_upper = per_genome_per_gen + ci_95,
         ci_lower = per_genome_per_gen - ci_95,
         ci_lower_max = max(c(per_genome_per_gen - ci_95), 1e-6)) |> 
  ungroup() |> #View()
   ggplot(aes(x = type, group = Species)) +
  scale_y_log10(guide = "axis_logticks", labels = label_log(),  expand = c(0, 0), limits = c(1e-6, 1e-1)) +
  coord_cartesian() +
  scale_x_discrete(expand = expansion(mult = c(0.25, 0.25))) +
  scale_color_manual(values = ct) +
  theme(legend.text = element_markdown(),
...
mut_totals <- 
  small_mutation_table |>
  dplyr::select(species, line, mut_type, complex_mutation) |>
  mutate(mut_type = case_when(mut_type == 'SNM' & complex_mutation ~ 'complex_SNM',
                              mut_type == 'SNM' & !complex_mutation ~ 'noncomplex_SNM',
                              TRUE ~ mut_type)) |> 
  dplyr::select(-complex_mutation) |>
  # add count for observed mutations
  mutate(mut_counter = 1) |>
  # add empty mutation rows to catch 0s
    rbind(expand.grid(line = all_lines, mut_type = c('complex_SNM', 'noncomplex_SNM', 'DEL', 'INS')) |>
...
coverage <- read.csv(here('data', 'coverage_stats.csv'))
sample_info <- read.csv(here('data', 'line_ids.csv')) |> rename('MA Line' = MA.Line, 'Sequence ID' = Sequence.ID) |>
  dplyr::select(-Species)

table_S1 <-
  full_join(mut_totals, snm_values, by = c('species', 'line')) |>
  arrange(species, line) |>
  dplyr::rename(Species = species,
                Line = line,
                'Total Non-complex SNM' = mut_total_noncomplex_SNM,
                'Total Complex SNM' = mut_total_complex_SNM,                
                'Total INS' = mut_total_INS,
                'Total DEL' = mut_total_DEL,
                'Non-complex SNM Rate' = rate_per_site_noncomplex_SNM,
                'Complex SNM Rate' = rate_per_site_complex_SNM,
  ...
  dplyr::select(Species, Line,
                `Total Non-complex SNM`, `Non-complex SNM Rate`,
                `Total Complex SNM`, `Complex SNM Rate`,
                `Total INS`, `INS Rate`,
                `Total DEL`, `DEL Rate`,
  ...
  left_join(coverage |> dplyr::select(-rname, -X, -startpos) |> dplyr::rename(chrom_length = endpos),
            by = 'Line') |>
  dplyr::rename('Sequence ID' = Line) |>
  left_join(sample_info, by = 'Sequence ID') |>

  dplyr::select(
    Species, `MA Line`, `Sequence ID`, starts_with('Total'), ends_with('Rate'), coverage, meandepth
  ) |>
  dplyr::rename('Sequencing Coverage (%)' = coverage,
                'Sequencing Depth' = meandepth) 
...
table_S2 <- 
small_mutation_table |>
  mutate(complex_mutation = case_when(mut_type != 'SNM' ~ NA,
                                      mut_type == 'SNM' ~ complex_mutation)) |>
  rename('Complex SNM' = complex_mutation,
         'Sequence ID' = line,
         'Species' = species,
...
table_s3 <-
  sv |>
  mutate(line = paste0(line, "_S1_L005")) |>
  rename("Sequence ID" = line) |>
  left_join(sample_info, by = "Sequence ID") |>
    mutate(Species = case_when(species == 'acidophilus' ~ 'Lb. acidophilus',
                             species == 'crispatus' ~ 'Lb. crispatus',
                             species == 'lactis' ~ 'Lc. lactis')) |>
  dplyr::select(Species, `MA Line`, `Sequence ID`, sv_type, transposition_element, start, end) 
...
table_1 <-
  ...
  full_join(fig_2a_data |>
            ...
            by = 's') |>
  full_join(table_S1 |>
  summarise(`Total Non-complex SNM` = sum(`Total Non-complex SNM`),
            `Total Complex SNM` = sum(`Total Complex SNM`),
  ...
  mutate(across(contains('Rate'), ~ signif(.x, 3)),
         ...
         ,`Effective Population Size` = round(`Effective Population Size`)
         ) |>
  dplyr::select(Species, `Chromosome Size`, `GC Content (%)`,
                ...
                `Total Non-complex SNM`, `SNM Rate (per Site per Generation)`, `SNM Rate SE`,
                `Total Complex SNM`,
...
```{r}
small_mutation_table |> 
  filter(mut_type %in% c('DEL', 'INS')) |>
  rowwise() |>
  mutate(mut_len = nchar(ALT) - nchar(REF)) |>
  ungroup() |>
  ggplot(aes(x = mut_len, fill = mut_type)) +
  geom_histogram() +
...
small_mutation_table |> 
  ...
  mutate(mut_len = nchar(ALT) - nchar(REF)) |>
  ungroup() |>
  summarise(mean_len = mean(mut_len),
            se_len = sd(mut_len) / sqrt(n()),
            .by = c('species', 'mut_type'))
```


# Lb. acidophilus mutator evolution
...
a_tree <-
  read.tree(
    here(
      'data',
      'ufboot.parsnp.cfml.treefile')) |>
  midpoint_root()


# set MA ancestor strain
maStrain <- 'GCF_034298135.1'
```


# GENE STATUS FROM ALIGNMENTS

```{r}
# IMPORT ALIGNMENTS

adaB_aln <-
  here('data', 'gene_alns', 'adaB.clean.aln') |> 
   readDNAStringSet()
adaB_aln <- adaB_aln[which( names(adaB_aln) != "GCF_051388635.1" )]
names(adaB_aln) <- 
lapply(names(adaB_aln), function(x){
  gsub('splitContig_', '', x)
})

alkD_aln <-
  here('data', 'gene_alns', 'alkD.clean.aln') |> 
   readDNAStringSet()
alkD_aln <- alkD_aln[which(!(names(alkD_aln) %in% c('GCF_051388555.1', 'GCF_051388695.1', 'GCF_051388835.1')) )]
names(alkD_aln) <- 
lapply(names(alkD_aln), function(x){
  gsub('splitContig_', '', x)
})

helD_aln <-
  here('data', 'gene_alns', 'helD.clean.aln') |> 
   readDNAStringSet()
helD_aln <- helD_aln[which( names(helD_aln) != 'GCF_051388635.1' )]
names(helD_aln) <- 
lapply(names(helD_aln), function(x){
  gsub('splitContig_', '', x)
})

mfd_aln <-
  here('data', 'gene_alns', 'mfd.clean.aln') |> 
   readDNAStringSet()

mutS_aln <-
  here('data', 'gene_alns', 'mutS.clean.aln') |> 
   readDNAStringSet()

## FILTER OUT NON INTACT SAMPLES & GET SAMPLES WITH SPECIFIC MUTATIONS

adaB_aln_filt <-
  adaB_aln[which(
    (subseq(adaB_aln, 16, 16) != '-') & # frame shift deletion
    (subseq(adaB_aln, 292, 292) != '-') # frame shift deletion
    )]

adaB_aln_del16 <- adaB_aln[which(subseq(adaB_aln, 16, 16) == '-')]
adaB_aln_del292 <- adaB_aln[which(subseq(adaB_aln, 292, 292) == '-')]


alkD_aln_filt <-
  alkD_aln[which(
    (subseq(alkD_aln, 147, 147) != '-') & # large deletion
    (subseq(alkD_aln, 176, 176) == 'G')   # premature stop
    )]

alkD_aln_del147 <- alkD_aln[which(subseq(alkD_aln, 147, 147) == '-')]
alkD_aln_stop176 <- alkD_aln[which(subseq(alkD_aln, 176, 176) == 'A')]

length(alkD_aln)
length(alkD_aln_filt)
length(alkD_aln_del147)
length(alkD_aln_stop176)

helD_aln_filt <-
  helD_aln[which(
    (subseq(helD_aln, 296, 296) != '-') &    # frame shift deletion
    (subseq(helD_aln, 1315, 1315) != 'T')   # premature stop
  #  (subseq(helD_aln, 2049, 2049) != '-') # frame shift deletion ( too late in protein to say its a real LOF mutation)
    )]

helD_aln_del296 <- helD_aln[which(subseq(helD_aln, 296, 296) == '-')]
helD_aln_stop1315 <- helD_aln[which(subseq(helD_aln, 1315, 1315) == 'T')]

mfd_aln_filt <-
  mfd_aln[which(
    (subseq(mfd_aln, 2001, 2001) != '-') & # frame shift deletion
    (subseq(mfd_aln, 2203, 2203) != '-') & # frame shift deletion
    (subseq(mfd_aln, 2593, 2593) != 'T')   # premature stop
  )]

mfd_aln_del2001 <- mfd_aln[which(subseq(mfd_aln, 2001, 2001) == '-')]
mfd_aln_del2203 <- mfd_aln[which(subseq(mfd_aln, 2203, 2203) == '-')]
mfd_aln_ps2593 <- mfd_aln[which(subseq(mfd_aln, 2593, 2593) == 'T')]

mutS_aln_filt <-
  mutS_aln[which(
    (subseq(mutS_aln, 56, 56) == '-') & # frame shift insertion
    (subseq(mutS_aln, 219, 219) == '-') &   # frame shift insertion
    (subseq(mutS_aln, 637, 637) != '-') & # frame shift deletion
    (subseq(mutS_aln, 651, 651) != 'T') # premature stop
  )]

mutS_aln_ins56 <- mutS_aln[which(subseq(mutS_aln, 56, 56) != '-')]
mutS_aln_ins219 <- mutS_aln[which(subseq(mutS_aln, 219, 219) != '-')]
mutS_aln_del637 <- mutS_aln[which(subseq(mutS_aln, 637, 637) == '-')]
mutS_aln_stop651 <- mutS_aln[which(subseq(mutS_aln, 651, 651) == 'T')]


gene_status <- 
  data.frame(sample = a_tree$tip.label) |>
    mutate(
      mutS = case_when(sample %in% names(mutS_aln_filt) ~ 'Intact', TRUE ~ 'Not Intact'),
      mfd = case_when(sample %in% names(mfd_aln_filt) ~ 'Intact', TRUE ~ 'Not Intact'),
      adaB = case_when(sample %in% names(adaB_aln_filt) ~ 'Intact', TRUE ~ 'Not Intact'),
      alkD = case_when(sample %in% names(alkD_aln_filt) ~ 'Intact', TRUE ~ 'Not Intact')#,
     # helD = case_when(sample %in% names(helD_aln_filt) ~ 'Intact', TRUE ~ 'Not Intact')
      ) |>
...
```{r}
context <- read.delim(here('data', 'gene_context.tsv'), header = F)
colnames(context) <- c('seqname', 'source', 'feature', 'start', 'end', 'score', 'strand', 'frame', 'attribute', 'sample', 'gene')

context <- 
  context |>
  filter(source == 'Panaroo') |>
  mutate(
    panaroo_gene = str_extract(attribute, "(?<=panaroo_gene_cluster=)[^;]+"),
    panaroo_gene = case_when(panaroo_gene == 'group_1602' ~ 'alkD', panaroo_gene == 'group_1539' ~ 'adaB', TRUE ~ panaroo_gene),
    arrow_forward = (strand == "+")) |>
  filter(sample == 'GCF_003952845.1',
         panaroo_gene %in% c('mutS', 'mfd', 'alkD', 'adaB'))  |>
...
mutations <-
  data.frame(
    panaroo_gene = c('mutS', 'mutS', 'mutS', 'mutS',
                     rep('mfd', 3),
                     'alkD',
                     'adaB'),
    aln_pos = c(56, 218, 635, 649,
                2001, 2203, 2593,
                176,
                16),
    mut_type = c('Frameshift', 'Frameshift', 'Frameshift', 'Premature Stop',
                 'Frameshift', 'Frameshift', 'Premature Stop',
                 'Premature Stop',
                 'Frameshift')
  ) |>
  left_join(context, by = 'panaroo_gene') |>
  mutate(plot_pos = 
           case_when(strand == "+" ~ start + aln_pos,
                     strand == "-" ~ end - aln_pos),
         placeholder = 0)
gene_plot <- 
  ggplot(context,
       aes(xmin = start, xmax = end, y = placeholder, fill = panaroo_gene, forward = arrow_forward)) +
  geom_segment(data = mutations,
               aes(x = plot_pos, y = placeholder, yend = placeholder + 0.5)
               ) +
  geom_point(data = mutations,
             aes(x = plot_pos, y = placeholder + 0.5,
                 shape = mut_type),
             size = 4,
             fill = 'white') +
    geom_gene_arrow() +
  # geom_text(data = mutations,
  #           aes(x = plot_pos, y = placeholder, yend = placeholder + 0.5,
  #               label = mut_type)) +
  facet_wrap(~ panaroo_gene, scales = "free", ncol = 1) +
  ...
  labs(
    shape = 'Mutation Type',
    fill = 'Gene',
    x = 'Position in GCF_003952845.1'
  )

gene_plot + canvas(7.5, 6.25)
...
```{r}
p <- ggtree(a_tree |> drop.tip(c(tree_subset(a_tree, 141, 0)$tip.label, tree_subset(a_tree, 129, 0)$tip.label)))  


p$data <-
  p$data |>
  mutate(
    blank = '', # blank column for doted lines without tip label
    
    adaB_var = 
      case_when(
        label %in% names(adaB_aln_del16) ~ 'Frameshift 16/518 bp',
        label %in% names(adaB_aln_filt) ~ 'Intact',
        TRUE ~ NA
      ),
    adaB_type = case_when(label != 'Intact' ~ 'Frameshift',
                          TRUE ~ NA),
    
    alkD_var = 
      case_when(
        label %in% names(alkD_aln_stop176) ~ 'Premature Stop 176/656 bp',
        label %in% names(alkD_aln_filt) ~ 'Intact',
        TRUE ~ NA
      ),
    alkD_type = case_when(label != 'Intact' ~ 'Premature Stop',
                          TRUE ~ NA),
    mfd_var = 
      case_when(
        label %in% names(mfd_aln_del2001) ~ 'Frameshift 2001/3497 bp',
        label %in% names(mfd_aln_del2203) ~ 'Frameshift 2203/3497 bp',
        label %in% names(mfd_aln_ps2593)  ~ 'Premature Stop 2593/3497 bp',
        label %in% names(mfd_aln_filt) ~ 'Intact',
        TRUE ~ NA
      ),
    mfd_type = case_when(grepl('Frameshift', mfd_var) ~     'Frameshift',
                         grepl('Premature Stop', mfd_var) ~ 'Premature Stop',
                          TRUE ~ NA),
    
    mutS_var = 
      case_when(
        label %in% names(mutS_aln_ins56) ~ 'Frameshift 56/2570 bp',
        label %in% names(mutS_aln_ins219) ~ 'Frameshift 218/2570 bp',
        label %in% names(mutS_aln_del637) ~ 'Frameshift 635/2570 bp',
        label %in% names(mutS_aln_stop651) ~ 'Frameshift 649/2570 bp',
        label %in% names(mutS_aln_filt) ~ 'Intact',
        TRUE ~ NA
      ),
    mutS_type = case_when(label != 'Intact' ~ 'Frameshift',
                          TRUE ~ NA))


p1 <- 
  p +
  geom_tiplab(aes(label = blank), align = TRUE) +
  coord_cartesian(clip = 'off') 

pi_samples <- read.csv(here('data', 'pi_results.csv')) |>
  filter(species == 'acidophilus', source == 'loo')  |>
  pull(left_out) |>
  str_replace(string = _, 'L_acidophilus_', '')
...
fig_3e <- 
  ...
  scale_fill_manual(values = c(ct[[2]], ct[[1]])) +
  geom_treescale(
    width = 0.001,
    linesize = 0.75,
    fontsize = 5,
...
Set up GPA Jaccard data

```{r}
a_gpa <-
  read_tsv(here('data', 
                'complete_acidophilus_gene_presence_absence.Rtab'))

c_gpa <-
  read_tsv(here('data', 
                'complete_crispatus_gene_presence_absence.Rtab'))

l_gpa <-
  read_tsv(here('data', 
                'complete_lactis_gene_presence_absence.Rtab'))
...
a_jaccard <- 
  ...
  as.matrix() |>
    melt(varnames = c("s1", "s2")) |>
  mutate(s1 = as.character(s1),
         s2 = as.character(s2)) |>
  filter(s1 > s2) |>
  mutate(spec1 = 'Lb_acidophilus')
...
l_jaccard <- 
  ...
  mutate(s1 = as.character(s1),
         s2 = as.character(s2)) |>
  filter(s1 > s2) |>
  mutate(spec1 = 'Lc_lactis')

jaccard <- 
  rbind(a_jaccard,
        c_jaccard,
        l_jaccard) |>
  rename(jaccard_dist = value) |>
  rowwise() |>
  mutate(s12 = paste(s1, s2, sep = ':', collapse = '')) |>
  ungroup() |>
  mutate(Species = case_when(str_detect(s12, 'acidophilus') ~ '*Lb. acidophilus*',
                           str_detect(s12, 'crispatus') ~ '*Lb. crispatus*',
                           str_detect(s12, 'lactis') ~ '*Lc. lactis*')) |>
  dplyr::select(s12, Species, jaccard_dist)
...
```{r}
a_ani <- read.delim(here('data', 'a_fastANI_out.txt'), header = FALSE)
colnames(a_ani) <- c('query', 'ref', 'ani', 'bidi_frag_maps', 'query_frags')
a_ani <-
  a_ani |>
  mutate(Species = '*Lb. acidophilus*',
         Assembly.Accession = paste0(query, '|', ref),
         Metric = 'ANI',
         value = ani) |>
  dplyr::select(Assembly.Accession, Species, Metric, value) 

c_ani <- read.delim(here('data', 'c_fastANI_out.txt'), header = FALSE)
colnames(c_ani) <- c('query', 'ref', 'ani', 'bidi_frag_maps', 'query_frags')
c_ani <-
  c_ani |>
  mutate(Species = '*Lb. crispatus*',
         Assembly.Accession = paste0(query, '|', ref),
         Metric = 'ANI',
         value = ani) |>
  dplyr::select(Assembly.Accession, Species, Metric, value) 

l_ani <- read.delim(here('data', 'l_fastANI_out.txt'), header = FALSE)
colnames(l_ani) <- c('query', 'ref', 'ani', 'bidi_frag_maps', 'query_frags')
l_ani <-
  l_ani |>
  mutate(Species = '*Lc. lactis*',
         Assembly.Accession = paste0(query, '|', ref),
         Metric = 'ANI',
         value = ani) |>
  dplyr::select(Assembly.Accession, Species, Metric, value) 
...
get and set up genome size and gene count statistics
```{r}

a <- read.delim(here('data', 'acidophilus_ncbi_assembly_data.tsv')) |> dplyr::select(Assembly.Accession, Annotation.Count.Gene.Protein.coding, Assembly.Stats.Total.Sequence.Length) |> mutate(species = 'a')
c <- read.delim(here('data', 'crispatus_ncbi_assembly_data.tsv')) |> dplyr::select(Assembly.Accession, Annotation.Count.Gene.Protein.coding, Assembly.Stats.Total.Sequence.Length) |> mutate(species = 'c')
l <- read.delim(here('data', 'lactis_ncbi_assembly_data.tsv')) |> dplyr::select(Assembly.Accession, Annotation.Count.Gene.Protein.coding,  Assembly.Stats.Total.Sequence.Length) |> mutate(species = 'l')

acl <- rbind(a,c) |> rbind(l) |>
    mutate(Species = case_when(species == 'a' ~ '*Lb. acidophilus*',
                           species == 'c' ~ '*Lb. crispatus*',
                           species == 'l' ~ '*Lc. lactis*'))  |>
  pivot_longer(c(Annotation.Count.Gene.Protein.coding, Assembly.Stats.Total.Sequence.Length),
               names_to = 'Metric',
               values_to = 'value') |>
  mutate(Metric = case_when(Metric == 'Annotation.Count.Gene.Protein.coding' ~ 'Annotated Protein Coding Genes',
                            Metric == 'Assembly.Stats.Total.Sequence.Length' ~ 'Assembly Length')) |>
  dplyr::select(-species)
...
acl <- 
  rbind(acl,
        ...
        jaccard |>
          mutate(Metric = 'Gene Content Jaccard Distance\n(All Pairwise Distances)', Assembly.Accession = s12) |>
          dplyr::rename('value' = jaccard_dist) |> 
          dplyr::select(Assembly.Accession, Species, Metric, value) 
  ) |>
  rbind(a_ani |> mutate(Metric = 'Average Nucleotide Identity\n(All Pairwise Comparisons)')) |>
  rbind(c_ani |> mutate(Metric = 'Average Nucleotide Identity\n(All Pairwise Comparisons)')) |>
  rbind(l_ani |> mutate(Metric = 'Average Nucleotide Identity\n(All Pairwise Comparisons)')) |>
  mutate(Species = case_when(
    Species == '*Lb. acidophilus*' ~ '*Lb. acidophilus* (n = 21)',
    Species == '*Lb. crispatus*' ~ '*Lb. crispatus* (n = 26)',
    Species == '*Lc. lactis*' ~ '*Lc. lactis* (n = 50)'))

  
metrics <- unique(acl$Metric)
t <- lapply(metrics, function(x){
  d <- acl |> filter(Metric == x) |> dplyr::select(Species, value)
  kt <- kruskal.test(value ~ Species, data = d)
  dt <- dunn.test(x = d$value, d$Species, method = "bonferroni")
  dt_df <- data.frame(comp = dt$comparisons, dunn.p.adj = dt$P.adjusted) |> mutate(kruskal.p = kt$p.value, metric = x)
  
  return(dt_df)
})  |> bind_rows() |>
  mutate(sig_kruskal = kruskal.p <= 0.05,
         sig_dunn = dunn.p.adj <= 0.05)
t
...
div_plot <-
  ...
  theme(
      ...
      plot.tag.position = c(0.78, 0.35)
      
  ) +
  #stat_summary(geom = 'point', fun = 'median', size = 5, shape = 21, fill = 'white') +
  facet_wrap(~Metric, scales = 'free') +
       labs(tag = 'All metrics were compared between\nspecies with the Kruskal-Wallis test and\nDunn post-hoc test. For all comparisons, \nBonferroni-corrected Dunn test p < 0.01.') #+
  


div_plot + canvas(15,10)

acl |>
  summarise(
    metric_median = median(value),
    .by = c('Species', 'Metric')
  )
